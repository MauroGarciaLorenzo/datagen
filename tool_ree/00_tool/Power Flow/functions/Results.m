function obj = Results(grid)
    ...
    obj.global    = table('Size', [grid.nb 5],'VariableTypes', ["double","double","double","double","string"],'VariableNames', ["bus","Vm","theta","V","type"]);
    ...
    obj.global{:,"bus"} = (1:1:grid.nb)';
    obj.global{:,"Vm"} = grid.Vm;
    obj.global{:,"theta"} = theta;
    obj.global{:,"V"} = V;
    ...
    for row = 1:height(obj.global)
        if ismember(obj.global{row,"bus"},grid.pq)
            obj.global{row,"type"} = "PQ";
        elseif ismember(obj.global{row,"bus"},grid.pv)
            obj.global{row,"type"} = "PV";
        else 
            obj.global{row,"type"} = "slack";
        end
    end
end
