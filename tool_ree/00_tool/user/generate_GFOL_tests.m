...
% % voltage magnitude in converter reference
% Au_n1   = [0];
% Bu_n1   = [0 0];
% Cu_n1   = [0];
% Du_n1   = [u_qc0/(sqrt(u_qc0^2+u_dc0^2)) u_dc0/(sqrt(u_qc0^2+u_dc0^2))];
% Au_n1_x = {''};
% Au_n1_u = {'u_qc' 'u_dc'};
% Au_n1_y = {'Umag'};
% u_n1 = ss(Au_n1,Bu_n1,Cu_n1,Du_n1,'StateName',Au_n1_x,'inputname',Au_n1_u,'outputname',Au_n1_y);
...
Bp_calc=[0 0 0 0]; 
Cp_calc=[0];
Dp_calc=[u_q0 u_d0 is_q0 is_d0];
p_calc_x={''};
p_calc_u={is_q is_d 'u_q' 'u_d'};
p_calc_y={'P_pu'};
p_calc = ss(Ap_calc,Bp_calc,Cp_calc,Dp_calc,'StateName',p_calc_x,'inputname',p_calc_u,'outputname',p_calc_y);
...
Bq_calc=[0 0 0 0]; 
Cq_calc=[0];
Dq_calc=[-u_d0 u_q0 +is_d0 -is_q0];
q_calc_x={''};
q_calc_u={is_q is_d 'u_q' 'u_d'};
q_calc_y={'Q_pu'};
q_calc = ss(Aq_calc,Bq_calc,Cq_calc,Dq_calc,'StateName',q_calc_x,'inputname',q_calc_u,'outputname',q_calc_y);
...
% output_vars = {iq,id,w_vsc,'P_pu','Q_pu'};  
% SS_GFOL = connect(vc_l2g,is_g2l,u_g2l,vsc_pu,pll_n1,dang,fdroop_n1,u_n1,udroop_n1,p_n1,q_n1,is_n1,rl_n1,p_calc,input_vars,output_vars); 


%input_vars = {'vg_q','vg_d',REF_w,omega_ref}; 
%output_vars = {iq,id,w_vsc};  
%SS_GFOL = connect(rl_n1,vsc_pu,pll_n1,dang,u_g2l,input_vars,output_vars); % net + pll
%SS_GFOL = connect(vc_l2g,is_g2l,u_g2l,vsc_pu,pll_n1,dang,is_n1,rl_n1,p_calc,q_calc,input_vars,output_vars);  % net + pll + CL + pq
%SS_GFOL = connect(vc_l2g,is_g2l,u_g2l,vsc_pu,pll_n1,dang,fdroop_n1,u_n1,udroop_n1,p_n1,q_n1,is_n1,rl_n1,p_calc,q_calc,input_vars,output_vars); % net + pll + CL + pq + P-Q blocks

input_vars = {'vg_q','vg_d',is_qc_ref};
output_vars = {iq,id};  
SS_GFOL = connect(vc_l2g,is_g2l,u_g2l,vsc_pu,is_n1,rl_n1,input_vars,output_vars);  % net + CL 
