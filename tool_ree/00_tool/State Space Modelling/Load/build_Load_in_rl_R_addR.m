function Load = build_Load_in_rl_R_addR(nodeAC,number,Connectivity_Matrix,T_nodes,f,R,L,pf_bus,delta_slk)
        ...
        for i=1:1:size(T_nodes(T_nodes.Node==nodeAC,:),2)-1
            ...
            if not(ismissing(cadena_element))
                ...
                elseif isequal(element,"MMC")
                   ...
                   Dnus(:,j:j+1) = [1 0;0 1];
                   j=j+2;
                elseif isequal(element,"STATCOM")
                   str = split(T_nodes{nodeAC,i+1});
                   numberb =str(2);
                   unus{j}   = join(['STAT',num2str(numberb),'.idiffq']);
                   unus{j+1} = join(['STAT',num2str(numberb),'.idiffd']);
                   Dnus(:,j:j+1) = [1 0;0 1];
                   j=j+2;
