function [T_SG, T_Rsnub] = generate_SS_SG_fromNL(T_SG, initSG, Connectivity_Matrix, out_nolin, index0)

    for sg = 1:1:size(T_SG.NodeAC,1)   
        %% GET LINEARIZATION POINT
        
%         from non-linear simulation CUTRÍSSSIM (!!!)

        if sg == 1
            we_0     = out_nolin.we_nl(index0);    
            is_q0    = out_nolin.isq_nl(index0);
            is_d0    = out_nolin.isd_nl(index0); 
            ik1_q0   = out_nolin.ikq1_nl(index0); 
            ik2_q0   = out_nolin.ikq2_nl(index0);  
            ik_d0    = out_nolin.ikd_nl(index0);        
            if_d0    = out_nolin.ifd_nl(index0);               
            Te_0    = out_nolin.Te_nl(index0);     
            v_q0     = out_nolin.vq_nl(index0);  
            v_d0     = out_nolin.vd_nl(index0);       
            etheta_0 = out_nolin.theta_nl(index0);   

        elseif sg ==2
            we_0     = out_nolin.we_nl2(index0);    
            is_q0    = out_nolin.isq_nl2(index0);
            is_d0    = out_nolin.isd_nl2(index0); 
            ik1_q0   = out_nolin.ikq1_nl2(index0); 
            ik2_q0   = out_nolin.ikq2_nl2(index0);  
            ik_d0    = out_nolin.ikd_nl2(index0);        
            if_d0    = out_nolin.ifd_nl2(index0);               
            Te_0    = out_nolin.Te_nl2(index0);     
            v_q0     = out_nolin.vq_nl2(index0);  
            v_d0     = out_nolin.vd_nl2(index0);       
            etheta_0 = out_nolin.theta_nl2(index0);  
        end

        vnXq0 = out_nolin.v1q_pu(index0);  
        vnXd0 = out_nolin.v1d_pu(index0);  


        %% SET VARIABLES NAMES

        SGnum   = T_SG.Number(sg);
        NodeAC  = T_SG.NodeAC(sg);
        % Exciter
        ...
        vnXq = ['NET.vn' num2str(NodeAC) 'q'];    %'NET.vs1_q' 
        vnXd = ['NET.vn' num2str(NodeAC) 'd'];    %'NET.vs1_d'
