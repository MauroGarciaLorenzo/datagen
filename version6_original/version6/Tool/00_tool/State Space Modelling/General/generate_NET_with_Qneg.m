...
[connect_mtx, connect_mtx_PI, connect_mtx_rl, T_nodes] = generate_general_connectivity_matrix(T_NET,T_trafo, T_load, T_shunt, T_TH, T_SG, T_STATCOM, T_VSC, T_IPC, T_b2b, T_user);
...
for l=1:1:size(T_load,1)
    ...
    if bus <= size(connect_mtx_PI,1) && sum(connect_mtx_PI(bus,:)) % Load is connected to any PI-line
          ...
          if T_load.L(l) == 0 &&  T_load.C(l) == 0% R Load 
            ...
            load = build_Load_in_PI_R_addR(T_load.bus(l),T_load.number(l),T_load.R(l),results.bus(results.bus.bus==bus,:),delta_slk(T_load.Area(l))); % R is INPUT          
          elseif T_load.L(l)~=0 % RL load
             ...
             load = build_Load_in_PI_addR(T_load.bus(l),T_load.number(l),T_global.f_Hz(1),T_load.R(l),T_load.L(l),results.bus(results.bus.bus==bus,:),delta_slk(T_load.Area(l))); % R is INPUT
          elseif T_load.C(l)~=0 % RC load
              load = build_LoadC_in_PI_addR(T_load.bus(l),T_load.number(l),T_global.f_Hz(1),T_load.R(l),T_load.C(l),results.bus(results.bus.bus==bus,:),delta_slk(T_load.Area(l))); % R is INPUT
          end
    
    else % Load is connected to all RL-lines
        
        if T_load.L(l) == 0 &&  T_load.C(l) == 0 % R Load 
            load = build_Load_in_rl_R_addR(T_load.bus(l),T_load.number(l),connect_mtx_rl,T_nodes,T_global.f_Hz(1),T_load.R(l),results.bus(results.bus.bus==bus,:),delta_slk(T_load.Area(l))); % R is INPUT        
        elseif T_load.L(l)~=0 % RL Load
            load = build_Load_in_rl_addR(T_load.bus(l),T_load.number(l),connect_mtx_rl,T_nodes,T_global.f_Hz(1),T_load.R(l),T_load.L(l),results.bus(results.bus.bus==bus,:),delta_slk(T_load.Area(l))); % R is INPUT 
        elseif T_load.C(l)~=0 % RC load
