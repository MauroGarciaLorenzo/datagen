%% Generate NET graph
    ...
    for i=1:1:size(T_DC_NET,1)
        ...
        Madj(busf_ac,bust_ac) = 1;
        Madj(bust_ac,busf_ac) = 1;
        T_DC_NET.busdc_from(i) = T_DC_NET.bus_from(i);
        T_DC_NET.busdc_to(i) = T_DC_NET.bus_to(i);
        T_DC_NET.bus_from(i) = busf_ac;
        T_DC_NET.bus_to(i) = bust_ac;
    end   
    ...
    % type = 3 --> HVDC
    T_lines          = [T_NET(:,["number","bus_from","bus_to"]); T_trafo(:,["number","bus_from","bus_to"]);T_DC_NET(:,["number","bus_from","bus_to"])];
    T_lines.drawn    = zeros(height(T_lines),1);
    T_lines.type     = [zeros(height(T_NET),1); 2*ones(height(T_trafo),1);3*ones(height(T_DC_NET),1)]; 
    ...
    for idx_node = 1:length(nodes)
        ...
        n_in(idx_node) = sum(T_lines.bus_to == currentNode);
        n_out(idx_node) = sum(T_lines.bus_from == currentNode);
    end
...
while ~isempty(linesQueue)
    ...
    if ~ismember(currentNode,visited)
        ...
        N_in    = [T_NET{T_NET.bus_to == currentNode,'bus_from'}; T_trafo{T_trafo.bus_to == currentNode,'bus_from'}; T_DC_NET{T_DC_NET.bus_to == currentNode,'bus_from'}]; % bus_to --> currentNode 
        N_out   = [T_NET{T_NET.bus_from == currentNode,'bus_to'}; T_trafo{T_trafo.bus_from == currentNode,'bus_to'}; T_DC_NET{T_DC_NET.bus_from == currentNode,'bus_to'}]; % currentNode --> bus_from
        ...
        if ~isempty(N_pqv)                
                ...
                for idx_el = 1:n_elements                   
                    ...
                    switch name
                        ...
                        case "IPC"
                            %AC BUS
                            add_block('myLibrary/BUS', [fileName '/BUS-IPC-' num],'Position', [x y x+height_bus y+width_bus],'Orientation',orientation); %measurement VI
                            set_param([fileName '/BUS-IPC-' num],'LabelV',['V_IPC_' num],'LabelI',['I_IPC' num],'ShowName','off');
                            %DC BUS
                            h_bus = add_block('myLibrary/DCBUS', [fileName '/DCBUS-IPC-' num2str(currentNode)],'Position', [x y x+height_bus y+width_bus],'Orientation',orientation); %measurement VI
                            h_goto = add_block('simulink/Signal Routing/Goto',  [fileName '/Goto'], 'MakeNameUnique','on','Position',[x y x+5 y+5]);
                            set_param(h_goto,'GotoTag',['iDC' num],'TagVisibility','global','ShowName','off');
                            add_line(fileName,get_param(h_bus,'PortHandles').Outport(1), get_param(h_goto,'PortHandles').Inport(1))
                            h_goto = add_block('simulink/Signal Routing/Goto',  [fileName '/Goto'], 'MakeNameUnique','on','Position',[x y x+5 y+5]);
                            set_param(h_goto,'GotoTag',['vDC' num],'TagVisibility','global','ShowName','off');
                            add_line(fileName,get_param(h_bus,'PortHandles').Outport(2), get_param(h_goto,'PortHandles').Inport(1))
                            %IPC:
                            mode = T_IPC{T_IPC.number == str2double(num), "mode"}{:};
                            add_block(['myLibrary/' mode], [fileName '/IPC-' num],'Position', element_xy,'Orientation',orientation); %user
                            set_param([fileName '/IPC-' num],'num',num);
                            %Connect AC bus
                            hbusblock = get_param([fileName '/BUS-IPC-' num],'PortHandles');
                            hblock = get_param([fileName '/IPC-' num],'PortHandles');
                            connect_buses_gen_force(fileName,hbus,hbusblock,side)
                            add_line(fileName,hbusblock.LConn(1),hblock.RConn(1))
                            add_line(fileName,hbusblock.LConn(2),hblock.RConn(2))
                            add_line(fileName,hbusblock.LConn(3),hblock.RConn(3))
                            %Connect DC bus
                            hbusblock = get_param([fileName '/DCBUS-IPC-' num2str(currentNode)],'PortHandles');
                            hblock = get_param([fileName '/IPC-' num],'PortHandles');
                            %connect_DCbuses_gen_force(fileName,hbus,hbusblock,side)
                            add_line(fileName,hbusblock.LConn(1),hblock.LConn(1))
                            add_line(fileName,hbusblock.LConn(2),hblock.LConn(2))
                        case "user"
...
%for i = 1:1:size(element_slk,1)
%    switch element_slk{i}
%        case 'TH' % TH as reference 
%          fb = T_global.fb(T_global.Area == T_TH.Area(T_TH.bus == bus_slk(i)));
%          x0 = add_angleRef(fileName,x0,y0,dx,dy,deltax,fb,T_global.Area(num_slk(i)));
%        case 'SG'
%          x0 = add_SGRef(fileName,x0,y0,dx,dy,deltax,num_slk(i),T_global.Area(num_slk(i)));
%        case 'GFOR'
%          x0 = add_GFORRef(fileName,x0,y0,dx,dy,deltax,num_slk(i),T_global.Area(num_slk(i))); 
%    end
    %x0 = x0 + 4;
%    y0 = y0 + dy;
%end
...
for i = b'
    switch element_slk{i}
        case 'TH' % TH as reference 
          fb = T_global.fb(T_global.SyncArea == T_TH.SyncArea(T_TH.bus == bus_slk(i)));
          x0 = add_angleRef(fileName,x0,y0,dx,dy,deltax,fb,T_TH.SyncArea(T_TH.number(num_slk(i))));
        case 'SG'
          x0 = add_SGRef(fileName,x0,y0,dx,dy,deltax,num_slk(i),T_SG.SyncArea(T_SG.number(num_slk(i))));
        case 'GFOR'
          x0 = add_GFORRef(fileName,x0,y0,dx,dy,deltax,num_slk(i),T_VSC.SyncArea(T_VSC.number(num_slk(i)))); 
    end
    %x0 = x0 + 4;
    y0 = y0 + 5;
end
...
% -------------------------------------------------------------------------
    ...
    for io_idx   = 1:length(ports)
        ...
        if ~contains(io_port,'NET.i') %line currents --> skip
            ...
            elseif contains(io_port,'DC_NET.v')
    ...
    % Add Line-to-Line Voltages
     ...
     % Add DC Voltages
        ...
        y = add_annotation(fileName, 'DC Quantities', x0, y0, dx, dy);
    
        for bus_idx = 1:length(list_DCbuses)
            %DC voltage
             h_from = add_block('simulink/Signal Routing/From',  [fileName '/From'], 'MakeNameUnique','on','Position',[x0 y x0+3*dx y+dy]);
                      set_param(h_from,'GotoTag',['vDC' num2str(list_DCbuses(bus_idx))],'TagVisibility','global','ShowName','off');
             h_scope = add_block('simulink/Sinks/Scope',[fileName '/Scope'], 'MakeNameUnique','on','Position',[x0+4*dx y x0+5*dx y+dy],'ShowName','off'); 
                      add_line(fileName,get_param(h_from,'PortHandles').Outport(1), get_param(h_scope,'PortHandles').Inport(1))
             h_2work_vdc   = add_block('simulink/Sinks/To Workspace',[fileName '/To Workspace' '_vDC_' num2str(list_DCbuses(bus_idx))], 'Position',   [x0+7*dx      y+dy   x0+9*dx   y+2*dy]);
             set_param(h_2work_vdc,'VariableName',['vDC' num2str(list_DCbuses(bus_idx))],'SaveFormat','Array','SampleTime','sim_config.Tsample','ShowName','off');
                      add_line(fileName,get_param(h_from,'PortHandles').Outport(1), get_param(h_2work_vdc,'PortHandles').Inport(1))
             y = y+1.5*dy;
            %DC current
             h_from = add_block('simulink/Signal Routing/From',  [fileName '/From'], 'MakeNameUnique','on','Position',[x0 y x0+3*dx y+dy]);
                      set_param(h_from,'GotoTag',['iDC' num2str(list_DCbuses(bus_idx))],'TagVisibility','global','ShowName','off');
             h_scope = add_block('simulink/Sinks/Scope',[fileName '/Scope'], 'MakeNameUnique','on','Position',[x0+4*dx y x0+5*dx y+dy],'ShowName','off'); 
                      add_line(fileName,get_param(h_from,'PortHandles').Outport(1), get_param(h_scope,'PortHandles').Inport(1))
             h_2work_vdc   = add_block('simulink/Sinks/To Workspace',[fileName '/To Workspace' '_iDC_' num2str(list_DCbuses(bus_idx))], 'Position',   [x0+7*dx      y+dy   x0+9*dx   y+2*dy]);
             set_param(h_2work_vdc,'VariableName',['iDC' num2str(list_DCbuses(bus_idx))],'SaveFormat','Array','SampleTime','sim_config.Tsample','ShowName','off');
                      add_line(fileName,get_param(h_from,'PortHandles').Outport(1), get_param(h_2work_vdc,'PortHandles').Inport(1))
             y = y+1.5*dy;
    ...
    for bus_idx = 1:length(list_buses)
        ...
        h_angle     = add_block('simulink/Signal Routing/From',[fileName '/From'], 'MakeNameUnique','on','Position', [x0      y+3*dy    x0+3*dx   y+4*dy]);
                      set_param(h_angle,'GotoTag',['angle_ref' num2str(T_PF.SyncArea(T_PF.bus==list_buses(bus_idx)))],'TagVisibility','global','ShowName','off');
