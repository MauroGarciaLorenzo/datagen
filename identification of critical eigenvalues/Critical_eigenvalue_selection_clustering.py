...
def check_cluster_memberships(labels_reshape,k):
    for i in range(labels_reshape.shape[1]):
        for l in range(0,k):
            if len(np.where(labels_reshape[:,i]==k))==0:
                print('missing eigenvalue in cluster '+str(k))
...
ax.scatter(df_real_clean_max, df_imag_clean_max, label='Max Eigenvalues')
ax.legend(loc='lower center',bbox_to_anchor=(0.45, -0.65),fontsize=15, ncol=2)
ax.set_title('Modal Map Zoom', fontsize=25)
plt.show()
...
for i in range(len(selected_eig_imag)):
    imag_selected=selected_eig_imag.loc[i,selected_eig_imag.columns].dropna()
    ind_imag_selected=imag_selected.index
    
    real_selected=selected_eig_real.iloc[i][ind_imag_selected].dropna()
...
check_cluster_memberships(labels_reshape, k)
...
for i in range(labels_reshape_T.shape[0]):
    eig_cl_index= imag_selected_df.loc[i,labels_reshape_T[i]==2]
    
    
# this equation calculates the DI of all of the eigenvalues (not in groups)
DI_tot[i] = 1-(-np.arrayreal_parts/np.sqrt(real_parts**2+imag_parts**2))
DI_tot_min = min(DI_tot)
DI_tot_max[i] = max(DI_tot)
