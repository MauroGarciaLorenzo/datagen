...
def exec(n_samples, Dims, f, error,ax):
    ...
    for cell in range(len(grid)):
        subsamples[cell] = grid[cell].explore_cell(f, n_samples, None, error, 0, cell,ax) # for each cell in grid, explore_cell
...
class Cell():
    ...
    # @task(returns=1)
    def explore_cell(self, f, n_samples, entropy, error, depth, cell,ax):
        # f = compss_wait_on(f)
        ...
        self.subsamples.extend(subsamples)

        
        entropy, delta_entropy = eval_entropy(self.subsamples, self.entropy) # eval entropy. Save entropy and delta_entropy as an attribute of the class Cell
        ...
        if delta_entropy < 0 or not check_dims(self.dimensions, error): 
            ...
            for cell_child in range(len(children_subsamples)):
                children_subsamples[cell_child] = children[cell_child].explore_cell(f, n_samples, entropy, error, depth + 1, cell,ax)
