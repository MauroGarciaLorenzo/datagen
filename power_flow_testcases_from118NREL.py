...
Generators_NREL=pd.read_excel('./Setup118/Generators_NREL.xlsx')
Generators_NREL_onlyCIG=Generators_NREL.query('Pmax_CIG !=0 and Pmax_TOT_SG==0')

Generators=pd.read_excel('./Setup118/Generators_red.xlsx')
...
def res_gen(res, GenNames, df):
    path='./Setup118/input-files/Input files/RT/'+res+'/'
...
for zone in [1,2,3]:
    ...
    Loads_R=pd.read_csv('./Setup118/input-files/Input files/RT/Load/LoadR'+str(zone)+'RT.csv')
