# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 15:06:24 2026
...
load_year=pd.DataFrame(index = Loads.index, columns = ['PL_'+str(i) for i in range(1,len(loads_nrel)+1)])

for zone in [1,2,3]:
    for date in Loads.index:
        Pd=Loads.loc[date,'R'+str(zone)]
        Pl=np.array(loads_nrel.query('Region == "R'+str(zone)+'"')['Load_Participation_Factor']*Pd).reshape(1,-1)
        load_year.loc[date,['PL_'+str(int(i)) for i in loads_nrel.query('Region == "R'+str(zone)+'"')['Num']]]=Pl
...
#%%
res=['Wind', 'Solar']
nodes_of_connections_RES = Generators.query('Type == @res')['Node_of_connection'].unique()
year_cig = pd.DataFrame(index = Wind.index, columns=nodes_of_connections_RES)

for node in nodes_of_connections_RES:
    generators_name_type = Generators.query('Node_of_connection == @node')[['GeneratorName','Type']]
    for type_ in generators_name_type['Type'].unique():
        if type_ == 'Wind':
            gen_name = [gen.replace(' 0','').replace(' ','') for gen in list(generators_name_type.query('Type == @type_')['GeneratorName'])]
            year_cig[node] = Wind[gen_name].sum(axis=1)
        if type_ == 'Solar':
            gen_name = [gen.replace(' 0','').replace(' ','') for gen in list(generators_name_type.query('Type == @type_')['GeneratorName'])]
            year_cig[node] = Solar[gen_name].sum(axis=1)
        
    
#%%

load_year.to_excel('Load_hourly_oneyear.xlsx')
year_cig.to_excel('CIG_hourly_oneyear.xlsx')
