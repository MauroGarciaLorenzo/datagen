...
def import_and_clean(foldername):
    ...
    df_real_clean = df_real_clean.drop([df_real_clean.columns[0]], axis=1).reset_index(drop=True)#, 'case_id', 'Stability'
    df_imag_clean = df_imag_clean.drop([df_imag_clean.columns[0]], axis=1).reset_index(drop=True)# , 'case_id', 'Stability'
...
#%%    
    ...
    #%%
[df_real_clean, df_imag_clean]=import_and_clean('../results/MareNostrum/datagen_ACOPF_slurm23172357_cu10_nodes32_LF09_seed3_nc3_ns500_d7_20250627_214226_7664')
...
df_real_remaining, df_real_subsamples, df_imag_remaining, df_imag_subsamples = train_test_split(df_real_clean, df_imag_clean, test_size = n_subsamples, shuffle = True, random_state = 42) 

#%%
# select the eigenvalues that have the highest real parts in each row 
df_real_subsamples_max = df_real_subsamples[df_real_subsamples.columns[0:2]]
df_imag_subsamples_max = df_imag_subsamples[df_imag_subsamples.columns[0:2]]
...
plt.grid()
ax.scatter(df_real_subsamples.drop(['case_id', 'Stability'],axis=1),df_imag_subsamples.drop(['case_id', 'Stability'],axis=1), label='Eigenvalues')
ax.scatter(df_real_subsamples_max, df_imag_subsamples_max, label='Max Eigenvalues')
...
[real_selected_df, imag_selected_df, X] = get_clustering_region_data(x_region, y_region, df_real_subsamples.drop(['case_id', 'Stability'],axis=1).reset_index(drop=True), df_imag_subsamples.drop(['case_id', 'Stability'],axis=1).reset_index(drop=True))
