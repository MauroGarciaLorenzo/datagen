...
#        self.sample=sample
    ...
    def adjust_within_boundaries(self,norm_samples):
        for i in range(0,self.n_subsamples):
            if any(norm_samples[i,:]>ub):
                ind=norm_samples[i,:]>ub
                diff_excess=norm_samples[i,ind]-np.array(ub)[ind]
                tot_excess=sum(diff_excess)
                diff_deficit=np.array(ub)[~ind]-norm_samples[i,~ind]
                excess_x_var=tot_excess/len(diff_deficit)
            
            norm_samples[i,ind]=norm_samples[i,ind]-diff_excess
            
            for j in np.arange(0,len(self.variables))[~ind]:
                norm_samples[i,j]=min(ub[j],norm_samples[i,j]+excess_x_var)
