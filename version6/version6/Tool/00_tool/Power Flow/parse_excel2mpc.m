function mpc = parse_excel2mpc(shared_power)
    ...
    for idx = 1:height(bus)     
        ...
        elseif any(contains(T_nodes{T_nodes.Node == busNum,:},"Load")) 
            bus(idx,2) = 1;  
        % For converters, decide bus based on the type field
        ...
        for idx = 1:height(bus)
            ...
            if T_shunt.state(shunt_number)==1
                if T_shunt{shunt_number,"type"} == "RLC"
                  ...
                  Z_C(idx)=-1j/(2*pi*T_global.f_Hz(T_shunt.Area(shunt_number))*T_shunt.C(shunt_number))/T_shunt.Zbpu_l2g(shunt_number);
                 
                  Z_shunt(idx)=Z_R(idx)+Z_L(idx)+Z_C(idx);
            ...
            if ~isempty(T_load)
               if T_load{T_load.bus == idx,"type"} == "RX"   
                   if T_load{T_load.bus == idx,"R"}
                      bus(idx,5) =bus(idx,5)+ P_shunt(idx); 
                   end
                   if T_load{T_load.bus == idx,"X"}
                      bus(idx,6) =bus(idx,6)+ Q_shunt(idx); 
                   end
               end
            else
                bus(idx,5) = P_shunt(idx);
                bus(idx,6) = Q_shunt(idx);
            end
        end
       end
