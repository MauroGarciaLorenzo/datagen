%% Generate NET graph
    ...
    graphPlot = plot(NET_graph,"NodeLabel",nodeNames,'Layout','force','MarkerSize',4,'LineWidth',1,'UseGravity',true,'Visible','off');
        ...
        if ~isempty(N_pqv)                
                ...
                for idx_el = 1:n_elements                   
                    ...
                    switch name
                        ...
                        case "Shunt"
                            %if T_shunt.state(num)==1
                                add_block('myLibrary/BUS', [fileName '/BUS-Shunt-' num],'Position', [x y x+height_bus y+width_bus],'Orientation',orientation); %measurement VI
                                set_param([fileName '/BUS-Shunt-' num],'LabelV',['V_Shunt_' num],'LabelI',['I_Shunt' num],'ShowName','off');
                                type = T_shunt{T_shunt.number == str2double(num), "type"}{:};
                                add_block(['myLibrary/' type], [fileName '/Shunt-' num],'Position', element_xy,'Orientation',orientation); % all other shunts                       
                                % % if T_shunt{T_shunt.number ==
                                % str2double(num),"L"} == 0 && strcmp(type,'RLC') This is NOT necessary as Simpowersystems allows setting L=0
                                % %     add_block('myLibrary/RC', [fileName '/Shunt-' num],'Position', element_xy,'Orientation',orientation); % RC shunt
                                % % else
                                % %     add_block(['myLibrary/' type], [fileName '/Shunt-' num],'Position', element_xy,'Orientation',orientation); % all other shunts                       
                                % % end
                                set_param([fileName '/Shunt-' num],'num',num);
                                hbusblock = get_param([fileName '/BUS-Shunt-' num],'PortHandles');
                                hblock = get_param([fileName '/Shunt-' num],'PortHandles');
                                connect_buses_load_force(fileName,hbus,hbusblock,side)
                                add_line(fileName,hbusblock.RConn(1),hblock.LConn(1))
                                add_line(fileName,hbusblock.RConn(2),hblock.LConn(2))
                                add_line(fileName,hbusblock.RConn(3),hblock.LConn(3))
...
% -------------------------------------------------------------------------
            ...
            elseif contains(io_port,'NET.Rl') %load input
                ...
                switch elementName
                    ...
                    case  'USER'
                        ...
                        list_pqv = [list_pqv; cell2table({str2double(num),bus,elementName},'VariableNames',{'num', 'bus', 'elementName'})]; 
                        list_buses(end+1) = bus;
                    case {'REF_w','GFOL','GFOR','STATCOM','Trafo'}
                        % pass
                    otherwise
    ...
    for pqv_idx = 1:height(list_pqv)
        if ~any(strcmp({'Load','TH','Shunt'},list_pqv.elementName{pqv_idx}))
            bus        = list_pqv.bus(pqv_idx);
            num        = list_pqv.num(pqv_idx);
