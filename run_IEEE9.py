...
dimensions = {'p_sg':
    Dimension(variables=p_sg, n_cases=n_cases, divs=2, borders=(d_op['Generators']['Pmin'].sum(), d_op['Generators']['Pmax_SG'].sum()),
              label=True,cosphi=generators_power_factor),
    'p_cig':
    Dimension(variables=p_cig, n_cases=n_cases, divs=1, borders=(d_op['Generators']['Pmin'].sum(), d_op['Generators']['Pmax_CIG'].sum()),
              label=True,cosphi=generators_power_factor),
    'perc_gfor':          
    Dimension(variables=[], n_cases=n_cases, divs=1, borders=(0, 1),
              label=True,cosphi=None),
    'p_gfor':
    Dimension(variables=p_cig, n_cases=n_cases, divs=1, borders=(0,0),
              label=False,cosphi=generators_power_factor),
    'p_gfol':
    Dimension(variables=p_cig, n_cases=n_cases, divs=1, borders=(0,0),
              label=False,cosphi=generators_power_factor),
    'p_load':
    Dimension(variables=p_loads, n_cases=n_cases, divs=1, borders=(0,0),
              label=False,cosphi=loads_power_factor),    
    }
