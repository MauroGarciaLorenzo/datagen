...
# %%
path = 'D:/'
dir_name=[dir_name for dir_name in os.listdir(path) if '_2732' in dir_name and 'zip' not in dir_name][0]#_2862# if dir_name.startswith('datagen') and 'zip' not in dir_name]#
print(dir_name)
...
for type_corr_analysis in ['']: #_HierCl
    ...
    X = df.drop(['case_id','Stability','cell_name','slack_bus'],axis=1).reset_index(drop=True)#[['V1','V2']]
...
#%%
for type_corr_analysis in ['','_HierCl']:
    theta_cols_uncorr = [c for c in df_dict['DataSet_training_uncorr_var'+type_corr_analysis+dataset_ID].columns if c.startswith('theta')]
    results_dataframes['uncorr_var'+type_corr_analysis]= df_dict['DataSet_training_uncorr_var'+type_corr_analysis+dataset_ID].drop(theta_cols_uncorr, axis=1).reset_index(drop=True)
    results_dataframes['uncorr_var'+type_corr_analysis] = pd.concat([results_dataframes['uncorr_var'+type_corr_analysis],df_taus_fixed.drop('Stability',axis=1),
                                                theta_rad_slack_26[theta_cols_uncorr].reset_index(drop=True)],axis=1)
...
for type_corr_analysis in ['','_HierCl']:
    ...
    #df = df_dict['DataSet_training_uncorr_var'+type_corr_analysis+dataset_ID]
    #df = results_dataframes['raw_data_slack_26']
    stab_cols=[c for c in results_dataframes['uncorr_var'+type_corr_analysis].columns if c.startswith('Stab')]
    X = np.array(results_dataframes['uncorr_var'+type_corr_analysis].drop(['case_id']+stab_cols,axis=1))
    Y = results_dataframes['uncorr_var'+type_corr_analysis][['Stability']].reset_index(drop=True).values.astype(int).ravel()
