...
#%%
df_cell_info = pd.read_csv(path+'/'+dir_name+'/cell_info.csv')
...
for internal_leaf in internal_leaves:
    ...
    try:
        ...
        for child in childs:
            ...
            cell_case_id[internal_leaf].extend(list(cell_case_id[child])[0:len(cell_case_id[child])-n_cases_x_n_samples])
            cell_case_id[child] = list(cell_case_id[child])[-n_cases_x_n_samples:]  
