...
fig.legend(handles, labels, loc='lower center', bbox_to_anchor=(0.5, -0.01), ncol=4, columnspacing=0.5)
...
for dataset_ID in dataset_ID_list:
    ...
    fig, ax = plt.subplots()
    ax.scatter(dimensions_caseid_unfeasible1_DS[dataset_ID]['p_cig'], dimensions_caseid_unfeasible1_DS[dataset_ID]['p_sg'],color='silver', label='Infeasible OP')
    ax.scatter(dimensions_caseid_unfeasible2_DS[dataset_ID]['p_cig'], dimensions_caseid_unfeasible2_DS[dataset_ID]['p_sg'],color='k', label='Feasible discarded OP')
...
for i, dir_name in enumerate(dir_names):
    path2dataset = Path(path +dir_name)

    mesh_df_files = [f for f in path2dataset.iterdir()
               if f.is_file() and f.name.startswith('mesh') and f.suffix in ['.xlsx', '.xls']]

    mesh_df_DS[dataset_ID_list[9]]= pd.read_excel('mesh'+dataset_ID+'.xlsx')
...
# Create 1 row, 2 columns of subplots, sharing the Y axis
fig, axes = plt.subplots(1, 2, sharey=True, figsize=(5*num_datasets, 4))
...
plt.savefig('figures_paper/entropy'+dataset_ID_list[0]+dataset_ID_list[1]+'.pdf', format='pdf')
plt.savefig('figures_paper/entropy'+dataset_ID_list[0]+dataset_ID_list[1]+'.png',dpi=320)#, format='pdf')
