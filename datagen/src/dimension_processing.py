...
def process_dimension(samples_df, dim, perc_gfor, generator):
    if dim.label == "p_cig":
        ...
        return process_other_dimensions(samples_df, dim, generator)
...
def process_p_cig_dimension_fixed_conf(samples_df, p_cig, perc_gfor, generator):
    """ Assigns values to g_for and g_fol dimensions.
    ...
    for _, sample in samples_df.iterrows():
        ...
        g_for_mulipliers = [1 if gfor == 1 else 0 for gfor in perc_gfor.values]
        cases_g_for = np.array(cases_p_cig_df * g_for_mulipliers)
        dims_g_for = cases_g_for.sum(axis=1)

        g_fol_mulipliers = [0 if gfor !=0 else 1 for gfor in perc_gfor.values]
        cases_g_fol = np.array(cases_p_cig_df * g_fol_mulipliers)
        dims_g_fol = cases_g_fol.sum(axis=1)

        cases_g_for_df = pd.DataFrame(
            cases_g_for,
            columns=[f"p_g_for_Var{v}" for v in range(len(p_cig.variable_borders))])
        dims_g_for_df = pd.DataFrame(dims_g_for.reshape(-1,1), columns=["p_g_for"])
        
        cases_g_fol_df = pd.DataFrame(
            cases_g_fol,
            columns=[f"p_g_fol_Var{v}" for v in range(len(p_cig.variable_borders))])
        dims_g_fol_df = pd.DataFrame(dims_g_fol.reshape(-1,1), columns=["p_g_fol"])
