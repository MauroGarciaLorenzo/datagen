...
def feasible_power_flow_ACOPF(case, **kwargs):
    ...
    fAndG = func_params.get("fAndG", None)
    opf_obj_fun = func_params.get("opf_obj_fun", None)
...
#%%
   ...
   #voltage_profile_list_complex = np.array([complex(v,0) for v in voltage_profile_list])
    ...
    opf_options = gce.OptimalPowerFlowOptions(solver=gce.SolverType.NR, verbose=0, ips_tolerance=1e-6, ips_iterations=50)
...
def optimal_power_flow_scipy(case, **kwargs):
    """
    Runs the alternating current optimal power flow (ACOPF) stability analysis.
    :param case: pandas DataFrame with the case parameters
    :param kwargs: dictionary with additional parameters
    ...
    d_opf = process_powerflow.update_OP(copy.deepcopy(gridCal_grid), opf_sol_completa, d_raw_data)
